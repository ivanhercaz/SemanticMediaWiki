...
class SomePropertyInterpreter {
	...
	public function interpretDescription( SomeProperty $description, $isConjunction = false, $isChain = false ) {
		...
		if ( $desc instanceof ValueDescription ) {
			$params = $this->interpretValueDescription( $desc, $property, $pid, $field, $isNot, $should );
		}

		// [[Foo::+]]
		if ( $desc instanceof ThingDescription ) {
			$params = $this->interpretThingDescription( $property, $pid, $field );
		}
		...
		$params = $this->fieldMapper->bool( ( $isNot ? 'must_not' : ( $should ? 'should' : 'must' ) ), $params );
	...
	private function interpretThingDescription( $property, $pid, $field ) {
	...
	private function interpretValueDescription( $desc, $property, $pid, &$field, &$isNot, $should ) {
		...
		$isNot = $comparator === SMW_CMP_NLKE || $comparator === SMW_CMP_NEQ;
