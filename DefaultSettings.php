...
return array(
	...
	'smwgElasticsearchConfig' => [
		'index' => [
			'data' => __DIR__ . '/data/elastic/smw-data.json',
		...
		'query' => [
			...
			// Threshold when to store and use the index lookup
			// Similar to `$smwgQConceptCacheLifetime`
			//
			// @default 1h
			'concept.terms.lookup.cache.lifetime' => 60 * 60,

			// Define which fields should be used when a wide proximity (e.g.
			// [[~~Foo]] or [[in:Foo]]) search is requested.
			//
			// - "*.txtField" match all text fields
			// - "*.wpgField" match all page title fields
			//
			// ^ can be used to boost the relevancy on a specific field (i.e
			// "*.txtField^3" )
			'wide.proximity.match.fields' => [ "*.txtField", "*.wpgField" ],
